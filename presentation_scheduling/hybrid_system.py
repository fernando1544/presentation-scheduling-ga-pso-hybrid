...
def hybrid_system():
    ...
    # Ejecuta el algoritmo genético durante 100 generaciones
    ga_max_generations = 100  # Número máximo de generaciones para el algoritmo genético
    ...
    # Optimiza utilizando PSO
    best_candidate_flat, best_penalty_point = pso.optimize()
    best_candidate = best_candidate_flat.reshape(slot_no, presentation_no)  # Reestructura el mejor candidato a la forma original

    # Escribe los resultados
    constraint_counts = \
        penalty(best_candidate, presentation_presentation, presentation_supervisor, supervisor_preference)
    plot_data = np.concatenate([ga_plot_data])  # Combina los datos de gráficos del algoritmo genético
